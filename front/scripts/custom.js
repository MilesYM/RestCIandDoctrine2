$(document).ready(function(){
	...
	// get the free screen height
	var x = ( $(document).height() - 70 );
	var y = ( $(document).height() - 290 );

	// profile map (extended by default)
	$('#profileMap').css('height', y +'px');

	// home page map
	var toggleState = 'up';

	$('#toggle-localisationMap').toggle(
		
		function(){
			$('#localisationMap').animate({height: x+'px'}, 500);
			$('#toggle-localisationMap img').attr('src','images/icons/arrow_up.png');
		},
  		function(){
			$('#localisationMap').animate({height: '160px'}, 500);
			$('#toggle-localisationMap img').attr('src','images/icons/arrow_down.png');
		});
});